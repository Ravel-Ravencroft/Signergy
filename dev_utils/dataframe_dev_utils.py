...
def _process_video(video_name: str, model: Holistic) -> Status:
	...
	try:
		...
		logging.info(msg=f'Extracted Landmarks from `{video_name}` Sign Video and Placed in `{sub_folder_path}`!')
		return Status.COMPLETE

	except Exception as e:
		logging.error(msg=f'An Error Occured while Processing Sign Video `{video_name}`:\n\n{e}')
		return Status.FAILED


def process_data(model: Holistic) -> Status:
	if (not TRIMS_DIR.is_dir()):
		logging.error(msg=f'The Source Directory `{TRIMS_DIR.parent.name}/{TRIMS_DIR.name}` Does Not Exist!')
		return Status.MISSING

	trims = {file.name.removesuffix('.mp4') for file in TRIMS_DIR.rglob('*.mp4')}

	if (not trims):
		logging.error(msg=f'The Source Directory `{TRIMS_DIR.parent.name}/{TRIMS_DIR.name}` is Empty!')
		return Status.EMPTY
	...
	if (not new_files):
		logging.info(msg='No New Files to Process!')
		return Status.EXISTS

	file_count = len(new_files)
	logging.info(msg=f'Extracting Landmarks from New Files: {file_count} Files Detected!')

	{_process_video(video_name=file, model=model) for file in new_files}
...
def create_dataframe(overwrite: bool = False) -> Status:
	if ((DATA_DIR / 'dataset.pkl').is_file() and not overwrite):
		logging.info(msg=f'Dataset Pickle Already Exists!')
		return Status.EXISTS
	...
	logging.info(msg='Dataset Pickle Created!')
	return Status.COMPLETE
...
def create_sign_dataframes(overwrite: bool = False) -> Status:
	...
	for sign in SIGNS_DIR.glob('*'):
		if ((sign / f'{sign.name}.pkl').is_file() and not overwrite):
			logging.info(msg=f'{sign.name} Dataset Pickle Already Exists!')
			continue
		...
		logging.info(msg=f'{sign.name} Dataset Pickle Created!')
...
def create_custom_dataframe(
	...
	if (not signs and not per_sign_count):
		logging.error(msg=
			f'Please Specify Either a Sign List or a Per-Sign-Max-Count to Use this Functionality!'
		)
		return Status.MISSING

	if (name and (DATA_DIR / f'{name}.pkl').is_file() and not overwrite):
		logging.error(msg=f'`{name}` Dataset Pickle Already Exists!')
		return Status.EXISTS
	...
	try:
		...
		logging.info(msg=f'`{df_name}` Dataset Pickle Created!')
		return Status.COMPLETE

	except Exception as e:
		logging.error(msg=f'An Error Occured while Creating `{df_name}` Dataset Pickle:\n\n{e}')
		return Status.FAILED
